...
- `DocumentDialog`:
  - Выбор PDF/изображения.
  - Предпросмотр документа.
  - Кнопка «Распознать» (вызывает `OCRService.process_document`).
  - Таблица с позициями и подсветкой `suspicious_fields`.
  - Кнопка «Создать приёмку» (вызов `SyncService.create_reception` + загрузка документа).

- `ResultsWidget`:
  - Таблица позиций приёмки.

- `ControlDialog`:
  - Список позиций для контроля.
  - Инструкции по контролю.
  - Встраиваемый `VideoWidget` для превью камеры.
  - Кнопки «Начать контроль», «Прошёл», «Не прошёл».
  - По завершении — остановка записи, загрузка видео, отправка `control-results`.
